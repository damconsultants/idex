...
class UpdateAllSku
{
   ...
   /**
    ...
    public function execute()
    {
        ...
        $productSku = $connection->fetchAll("SELECT * FROM {$table} WHERE status = 'pending' LIMIT 100");
        ...
        $collection_slug_val = $meta_properties['collection_data_slug_val'];
			...
			//echo "<pre>"; print_r($productSku); exit;
            //if (count($productSku) > 0) {
                foreach ($productSku as $skuData) {
                    $sku = $skuData['sku'];
                    if ($sku != "") {
                    ...
                    $connection->delete($table, ['id = ?' => $skuData['id']]);
                }
            //}
            //$connection->update($table, ['status' => 'processed'], ['id = ?' => $skuData['id']]);
            $result_data = $result->setData([
                'status' => 1,
    ...
    public function getDataItem($select_store, $select_attribute, $convert_array, $collection_data_slug_val, $current_sku)
    {
                ...
                if ($select_attribute == $data_value['type']) {
                    ...
                    if ($data_value['type'] == "image") {
						...
						*/
                        ...
                        $assets_extra_details_doc["assets_extra_details"][$data_value["idHash"]] = array_merge($data_value["assets_extra_details"], $b, $c, $f);
                        $doc_name = $data_value["name"];
                        $doc_name_with_space = preg_replace("/[^a-zA-Z]+/", "-", $doc_name);
                        $doc_link = $image_data["image_link"] . '@@' . $doc_name_with_space . "\n";
        ...
        if ($select_attribute == 'image' || $select_attribute == 'video' || $select_attribute == 'document') {
            ...
            if (count($data_arr) > 0) {
                $this->getProcessItem($select_store, $select_attribute, $data_arr, $data_val_arr, $bynder_extra_data);
            } else {
            ...
            if (count($data_arr) > 0) {
                $this->getProcessImageItem($select_store, $select_attribute, $data_arr, $data_val_arr, $bynder_extra_data, $bynder_extra_data_video, $type);
            }
            if (count($doc_data_arr) > 0) {
                $this->getProcessDocItem($select_store, $select_attribute, $doc_data_arr, $doc_data_val_arr, $bynder_extra_data_doc);
            }
        }
    }
    /**
     ...
     * @param array $bynder_extra_data
     * @return $this
     */
    public function getProcessItem($select_store, $select_attribute, $data_arr, $data_val_arr, $bynder_extra_data)
    {
        $result = $this->resultJsonFactory->create();
     ...
     * @param array $data_val_arr
     */
    public function getProcessImageItem($select_store, $select_attribute, $data_arr, $data_val_arr, $bynder_extra_data, $bynder_extra_data_video, $type)
    {
     ...
     * @param array $data_val_arr
     */
    public function getProcessDocItem($select_store, $select_attribute, $data_arr, $data_val_arr, $bynder_extra_data)
    {
        $image_value_details_role = [];
        $temp_arr = [];
        $byn_is_order = [];
     ...
     * @param string $select_attribute
     */
    public function getUpdateImageVideo(
        $img_json,
        $product_sku_key,
        $mg_role,
        $image_alt_text_value,
                ...
                if (!empty($image_value)) {
                    ...
                    if (count($merge_both_diff) > 0) {
                        ...
                        if ($select_store == 'all_store') {
                            ...
                            $this->getInsertMedaiDataTable($product_sku_key, $d_media_id, $product_ids, $select_store);
                        }
                    ...
                    if ($select_store == 'all_store') {
                        ...
                        $this->getInsertMedaiDataTable($product_sku_key, $m_id, $product_ids, $select_store);
                    }
                    $flag = 0;
                    ...
                    */
                    ...
                    $both_merge_extra_detail = array_merge($by_extra_details, $by_extra_details_video);
                    $update_latest_code = [
                        "asset_list" => $both_merge,
                        "assets_extra_details" => $both_merge_extra_detail
                    ];
                    $new_value_array = json_encode($update_latest_code, true);
                    $updated_values = [
                        'bynder_multi_img' => $new_value_array,
     ...
     * @param string $img_alt_text
     * @param string $bynder_media_ids
     * @param string $bynder_extra_data
     * @param string $byd_hash_id
    ...
    public function getUpdateImage(
        ...
        try {
            $storeIds = 0; //$this->storeManagerInterface->getStore()->getId();
            //echo "<pre>"; print_r($storeIds);
            //echo "<pre>"; print_r($select_store); exit;
            $_product = $this->_productRepository->get($product_sku_key);
            ...
            if ($select_attribute == "image") {
                if (!empty($image_value)) {
                    ...
                    if ($select_store == 'all_store') {
                        $this->getInsertMedaiDataTable($product_sku_key, $media_id, $product_ids, $storeIds);
                        $all_stores = $this->getMyStoreId();
                        if (count($all_stores) > 0) {
                            foreach ($all_stores as $storeId) {
                        ...
                        $this->getInsertMedaiDataTable($product_sku_key, $media_id, $product_ids, $select_store);
                    }
